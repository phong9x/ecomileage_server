  ...
  <div class="page-content">
    <div class="container">
      <div class="main-body-content member-page">
        ...
        <form id="form_bike_mileage" method="post">
          <div class="member-title-table">
            <div class="title-page full border-bottom mrb-30">
              <h3 class="bold pull-left">트랙킹 데이터 상세</h3>
            </div>
            <div class="full border-full pad-20-30 mrb-40">
              <div class="full mrb-10">
                <div class="full"><div id="map"></div></div>
                <div class="full" style="border: 1px solid #dddddd; margin-top: 24px;">
                 	<input type="hidden" name="id" value="${item.id}"> 
                  	<textarea name="memo" id="memo" rows="6" placeholder="메모란" class="rw-ckeditor form-control">${item.memo }</textarea>
                </div>
              </div>
              <div class="full text-center">
                <button type="button" class="btn red pad-8-50 bike-link-back">목록</button>
                <button type="button" class="btn green pad-8-50 bike-button-confirm">저장</button>
              </div>
            </div>
          </div>
        </form>
...
-->
<!-- <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyC7HfG1l1-gBrpRtTZcqwwoQNMNI4Xf-Us&callback=initMap"></script> -->
...
<script>
	...
	$('.bike-link-back').click(function(){
  		window.location = '/auth/mileage/bike/list';
  	});
	$('.bike-button-confirm').click(function(){
		var memo = $('#memo').val();
		console.log(memo);
		if (memo.leng == 0) {
			alert("Please input information.")
		} else {
			$('#form_bike_mileage').submit();
		}
