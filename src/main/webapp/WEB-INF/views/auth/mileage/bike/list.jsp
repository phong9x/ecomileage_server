            ...
            <div class="main-body-content member-page">
              ...
              <div class="member-title-table">
                <div class="title-page full border-bottom mrb-30">
                  <h3 class="bold pull-left">트랙킹 데이터</h3><span>주행 마일리지 데이터를 관리합니다.</span>
                </div>
              ...
              <div class="table-member-mng table-responsive">
              	<c:forEach items="${list}" varStatus="loop" var="i">
              		<table class="table table-striped table-bordered table-hover text-center" onclick="window.location.href='/auth/mileage/bike/edit?id=${i.id}'">
                  <tbody>
                    <tr>
                      <td rowspan="6">${totalCount - loop.index  - (currentIndex-1)*size}</td>
                      <td colspan="6">${i.fileName }</td>
                    </tr>
                    <tr>
                      <td>출발지</td>
                      <td>${i.latStart},${i.longStart} </td>
                      <td>이동중 평균속도</td>
        	...
        	$(document).ready(function(){
        		...
        		$('.btn-process-approval').click(function(){
              		...
              		$.ajax({
    					...
    					success:function(rs) {
    						if(rs.status == 200){
    							window .location.href = "/auth/mileage/bike/list";
    						} else {
    							alert("Can not change status !!!");
    						}
    					}
