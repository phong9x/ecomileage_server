            ...
            <div class="main-body-content member-page">
              ...
              <div class="member-title-table">
                <div class="title-page full border-bottom mrb-30">
                  <h3 class="bold pull-left">주행 마일리지 물품 신청</h3><span>주행 마일리지 물품 신청 리스트를 관리합니다.</span>
                </div>
                <div class="full border-full pad-20-30 mrb-40">
                 ...
                 <div class="full mrb-30">
                    ...
                    <div class="col-md-1">
                      <select name="status" title="상태" class="form-control selectpickermember text-left">
                        <option value="">상태</option>
                        <option value="0">검토중</option>
                        <option value="1">승인</option>
                        <option value="2">불인</option>
                      </select>
                    </div>
                    <div class="col-md-2 text-right line-30">물품명으로 검색<i aria-hidden="true" class="fa fa-angle-right font-18 mrl-10"></i></div>
                    <div class="col-md-2">
                      <input type="text" name="registerItem" value="${param.registerItem }" class="form-control"/>
                    </div>
                  </div>
                ...
                <table class="table table-striped table-bordered table-hover text-center">
                  ...
                  <tbody>
	                  <c:forEach items="${list }" var="i">
	                    <tr>
	                      <td><a href="/auth/mileageApply/bike/edit?id=${i.id}">${i.user.username}</a></td>
	                      <td>${i.user.fullname}</td>
	                      <td>${i.postCode}</td>
	                      <td>${i.address }<br>${i.addressDetail }</td>
	                      <td>${i.registerItem }</td>
	                      <td>${i.pointUse}</td>
	                      <td><fmt:formatDate value="${i.createDate }" pattern="yyyy-MM-dd"/></td>
	                      <td>
	                      	<c:if test="${i.status == 0}">검토중</c:if>
	                      	<c:if test="${i.status == 1}">승인</c:if>
	                      	<c:if test="${i.status == 2}">불인</c:if>
	                      	</td>
	                    </tr>
	                  </c:forEach>
